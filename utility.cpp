/*This file contains useful functions needed for functioning of other files*/
#include<iostream>
#include<fstream>
#include<string>
#include<sstream>
...
string intToHexString(int x,int fill = 5){
  stringstream s;
  s << setfill('0') << setw(fill) << hex << x;
  string t = s.str();
  t = t.substr(t.length()-fill,fill);
...
void read_first_nonwhitespace(string line,int& index,bool& status,string& data,bool readTillEnd=false){
  data = "";
  status = true;
  if(readTillEnd){
  ...
  while(index<line.length()&&!check_whitespace(line[index])){//If no whitespace then data
    data += line[index];
    index++;
  }
  ...
  while(index<line.length()&&check_whitespace(line[index])){//Increase index to pass all whitespace
    index++;
  }
}
...
void read_byte_operand(string line,int& index,bool& status,string& data){
  ...
  if(line[index]=='C'){
    ...
    while(index<line.length()&&!check_whitespace(line[index])){//In no whitespace then data
      data += line[index];
      index++;
    }
  }
