...
void handle_LTORG(string& litPrefix, int& lineNumberDelta,int lineNumber,int& LOCCTR, int& lastDeltaLOCCTR, int currentBlockNumber){
  ...
  for(auto const& it: LITTAB){
    ...
    if(litAddress!="?"){
      ...
      litPrefix += "\n" + to_string(lineNumber) + "\t" + intToHexString(LOCCTR) + "\t" + to_string(currentBlockNumber) + "\t" + "*" + "\t" + "="+litValue + "\t" + " " + "\t" + " ";
...
void evaluateExpression(string expression, bool& relative,string& tempOperand,int lineNumber, ofstream& errorFile,bool& error_flag){
  ...
  if(!Illegal){
    if(pairCount==1){
      ...
      tempOperand = intToHexString(tempOBJ.getResult());
    }
    else if(pairCount==0){
      /*absolute*/
      relative = 0;
...
void pass1(){
  ...
  read_first_nonwhitespace(fileLine,index,statusCode,label);
  read_first_nonwhitespace(fileLine,index,statusCode,opcode);


  if(opcode=="START"){
    read_first_nonwhitespace(fileLine,index,statusCode,operand);
    read_first_nonwhitespace(fileLine,index,statusCode,comment,true);
    startAddress = stringHexToInt(operand);
    // cout<<startAddress<<endl;
    // exit(0);
    LOCCTR = startAddress;
    writeData = to_string(lineNumber) + "\t" + intToHexString(LOCCTR-lastDeltaLOCCTR) + "\t" + to_string(currentBlockNumber) + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment;
    writeToFile(intermediateFile,writeData); // Write file to intermediate file

    getline(sourceFile,fileLine); //Read next line
    lineNumber += 5;
    index = 0;
    read_first_nonwhitespace(fileLine,index,statusCode,label); //Parse label
    read_first_nonwhitespace(fileLine,index,statusCode,opcode);//Parse OPCODE
  }
  else{
    startAddress = 0;
    LOCCTR = 0;
  ...
  while(opcode!="END"){  		
    ...
    if(!check_comment_line(fileLine)){
      ...
      if(OPTAB[getRealOpcode(opcode)].exists=='y'){//Search for opcode in OPTAB
        if(OPTAB[getRealOpcode(opcode)].format==3){
          ...
          if(getRealOpcode(opcode)=="RSUB"){
            ...
            if(operand[operand.length()-1] == ','){
              read_first_nonwhitespace(fileLine,index,statusCode,tempOperand);
              operand += tempOperand;
            }
          }
          ...
          if(getFlagFormat(operand)=='='){
            ...
            if(tempOperand=="*"){
              tempOperand = "X'" + intToHexString(LOCCTR-lastDeltaLOCCTR,6) + "'";
            }
        ...
        if(operand=="*"){
          tempOperand = intToHexString(LOCCTR-lastDeltaLOCCTR,6);
          relative = 1;
        }
        else if(if_all_num(operand)){
          tempOperand = intToHexString(stoi(operand),6);
          relative = 0;
        }
        else{
          char lastByte = operand[operand.length()-1];
      ...
      if(opcode=="EQU" && SYMTAB[label].relative == 0){
        writeData = writeDataPrefix + to_string(lineNumber) + "\t" + intToHexString(LOCCTR-lastDeltaLOCCTR) + "\t" + " " + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment + writeDataSuffix;
      } 
      else{
        writeData = writeDataPrefix + to_string(lineNumber) + "\t" + intToHexString(LOCCTR-lastDeltaLOCCTR) + "\t" + to_string(currentBlockNumber) + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment + writeDataSuffix;
    ...
    BLOCKS[currentBlockName].LOCCTR = intToHexString(LOCCTR);//Update LOCCTR of block after every instruction
    getline(sourceFile,fileLine); //Read next line
...
//*****************************************************************************************
  ...
  if(opcode!="END"){
     ...
     if(SYMTAB[label].exists=='n'){
  	...
  	writeToFile(intermediateFile, writeDataPrefix + to_string(lineNumber) + "\t" + intToHexString(LOCCTR-lastDeltaLOCCTR) + "\t" + to_string(currentBlockNumber) + "\t" + label + "\t" + opcode);
...
if(opcode=="END"){
  ...
  writeData = to_string(lineNumber) + "\t" + intToHexString(LOCCTR-lastDeltaLOCCTR) + "\t" + " " + "\t" + label + "\t" + opcode + "\t" + operand + "\t" + comment + writeDataSuffix;
  writeToFile(intermediateFile,writeData);
  ...
  for(auto const& it: BLOCKS){
    if(it.second.staAdr=="?"){
      BLOCKS[it.first].staAdr= intToHexString(LocctrArr[it.second.number - 1]);
    }
  }
